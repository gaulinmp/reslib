...
This module contains code to download and cache Compustat data from WRDS.
...
class CompustatAnnual(_DataFrameCache):
    ...
    def make_dataset(self):
        ...
        with _wrds_connection() as wrds:
            df_comp = wrds.read_sql(funda_query,
    ...
    def _post_read_hook(self, df: "_pd.DataFrame", **kwargs):
        # kwargs = {**self.postread_args, **kwargs}
        # Duplicate named column from sql read have .1 appended. Delete them.
        for c in df.filter(like='.1'):
            del df[c]

        df = df.sort_values('gvkey fyear'.split()).reset_index(drop=True)

        # Add lags
        for c in 'at seq mve sale epspx xrd xsga'.split():
            df[f'lag_{c}'] = _pd.merge(df['gvkey fyear'.split()],
        ...
        for c in 'at mve'.split():
            df.loc[df[c] != 0, f"log_{c}"] = df.loc[df[c] != 0, c].apply(_np.log)

        # Add simple eval-able calculations:
        equations = """
        ave_at             =  (at + lag_at) * 0.5
        ...
        roa_ib             =  ib / lag_at
        roa_oiadp          =  oiadp / lag_at
        roe_ib             =  ib / lag_seq
        roe_oiadp          =  oiadp / lag_seq
        ...
        equations = [x.strip() for x in equations.split('\n')
                        # filter out empty lines and comments
                     if (x.strip() or '#')[0] != '#']
        # Now add variables (np.inf replaced at the end)
        ...
        df["dividend"] = (df["dvc"].fillna(df["dv"]) + df["dv"].fillna(df["dvc"]) > 0).astype(float)
        # loss = 0; IF ib < 0 THEN loss = 1;
        df['loss'] = (df['ib'] < 0).astype(float)
        ...
        df.loc[df[cols].isnull().any(axis=1), 'life_cycle life_cycle_legend'.split()] = None

        df['firm_age'] = ((df.datadate - df.funda_start) / _pd.to_timedelta(1, 'D') / 365.25).round(2)
        ...
        return df.replace([_np.inf, -_np.inf], _np.nan)
